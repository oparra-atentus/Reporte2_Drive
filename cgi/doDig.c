/* $Id: doDig.c 1090 2005-09-09 15:04:29Z alberto $ */
...
main(int argc, const char **argv) {
	 ...
	 */
	execl("/usr/bin/ssh",
		  "/usr/bin/ssh", origen_calificado, "-l", "monitor", "/usr/bin/dig", "any", destino,
		  (char *)NULL);
...
parseargs(int argc, const char **argv) {
	...
	struct poptOption optionsTable[] = {
		{ "origen", 'o', POPT_ARG_STRING, &origen, 0, "servidor de origen de consulta", "servidor"},
		{ "destino",'d', POPT_ARG_STRING, &destino, 0, "host a consultar", "host"},
		POPT_AUTOHELP
		POPT_TABLEEND
	};
