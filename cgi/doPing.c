/* $Id: doPing.c 1093 2005-09-09 15:30:28Z alberto $ */
...
main(int argc, const char **argv) {
	...
	snprintf(numprobes, 15, "%d", NUMPROBES);
	 ...
	 */
	execl("/usr/bin/ssh", "/usr/bin/ssh", origen_calificado, "-l", "monitor", "/bin/ping", "-c", numprobes, destino, (char *)NULL);
...
parseargs(int argc, const char **argv) {
	...
	struct poptOption optionsTable[] = {
		...
		{ "destino",'d', POPT_ARG_STRING, &destino, 0, "host de destino de pings", "host"},
		POPT_AUTOHELP
		POPT_TABLEEND
	};
