...
function checkSubobjetivo(subobjetivo_id) {
	...
	if (document.getElementById("subobjetivo_"+subobjetivo_anterior))
    {
        ...
        document.getElementById("_subobjetivo_"+subobjetivo_anterior).className = "nada";
	}
	...
	if (document.getElementById("subobjetivo_"+subobjetivo_id))
    {
        ...
        document.getElementById("_subobjetivo_"+subobjetivo_id).className = "spriteImg spriteImg-bot_check";
...
var evento_anterior = 0;
function checkEvento(evento_id) {

	if (document.getElementById("evento_"+evento_anterior))
    {
		document.getElementById("evento_"+evento_anterior).className = "radiounselected";
        document.getElementById("_spriteevento_"+evento_anterior).className = "nada";
	}
	
	if (document.getElementById("evento_"+evento_id))
    {
		document.getElementById("evento_"+evento_id).className = "radioselected";
        document.getElementById("_spriteevento_"+evento_id).className = "spriteImg spriteImg-bot_check";
		document.form_principal.evento_id.value = evento_id;
		evento_anterior = evento_id;
	}
}
...
<input type="hidden" name="tipo_obj"   value="{__tipo_obj}" />
...
<table align="center" width="80%">
						...
						<table width="100%">
							...
							<tr>
								...
								<td height="30" id="subobjetivo_{__subobjetivo_id}" onclick="checkSubobjetivo('{__subobjetivo_id}');" class="radiounselected" style="font-size: 13px; padding: 0px 0px 0px 20px; cursor: pointer; font-family: Trebuchet MS, Verdana, sans-serif; border: solid 1px #a2a2a2; ">
                                    <i id="_subobjetivo_{__subobjetivo_id}" onclick="checkSubobjetivo('{__subobjetivo_id}');" class="nada" style="position: absolute;"></i>
	...
	<tr>
		<td>
			<table width="100%">
				<tr>
					<td style="padding: 5px; background-color: #a2a2a2; color: #ffffff; font-size: 15; font-family: Trebuchet MS, Verdana, sans-serif; font-weight: bold; text-transform: uppercase;">Eventos</td>
				</tr>
				<tr>
					<td align="center">
						<input name="evento_id" type="hidden" />
						<table width="100%">
							<!-- BEGIN LISTA_EVENTOS_TR -->
							<tr>
								<!-- BEGIN LISTA_EVENTOS_TD -->
								<td height="30" id="evento_{__evento_id}" onclick="checkEvento('{__evento_id}');" class="radiounselected" style="font-size: 13px; padding: 0px 0px 0px 20px; cursor: pointer; font-family: Trebuchet MS, Verdana, sans-serif; border: solid 1px #a2a2a2;">
                                    <i id="_spriteevento_{__evento_id}" onclick="checkSubobjetivo('{__evento_id}');" class="nada" style="position: absolute;"></i>
									<div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; width: 290px; float: left; padding: 0px 0px 0px 50px;">{__evento_nombre}</div>
								</td>
								<!-- END LISTA_EVENTOS_TD -->
							</tr>
							<!-- END LISTA_EVENTOS_TR -->
						</table>
					</td>
				</tr>
			</table>
			<script>
				checkEvento('{__evento_id_default}');
			</script>
		</td>
	</tr>
	<tr>
					...
					<td align="center">
						...
						<table width="100%">
							...
							<tr>
								...
								<td height="30" id="horario_{__horario_id}" onclick="checkHorario('{__horario_id}');" class="radiounselected" style="ffont-size: 13px; padding: 0px 0px 0px 20px; cursor: pointer; font-family: Trebuchet MS, Verdana, sans-serif; border: solid 1px #a2a2a2; ">
